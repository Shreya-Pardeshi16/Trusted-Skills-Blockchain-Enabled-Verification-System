...
public List<Institutes> getInstReport(){
		...
		rs=st.executeQuery("select * from institutes where usertype='institute' ");
		Institutes vs;
	...
	public String addNewInst()
	{
		...
		if(x>0) {
			...
			pst=con.prepareStatement("insert into institutes values(?,?,?,?,?,?,?,?,?,?  )");
